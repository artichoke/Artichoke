...
mod tests {
    ...
    fn rclass_for_undef_root_module() {
        ...
        let spec = Spec::new(&mut interp, "Foo", c"Foo", None).unwrap();
    ...
    fn rclass_for_undef_nested_module() {
        let mut interp = interpreter();
        let scope = Spec::new(&mut interp, "Kernel", c"Kernel", None).unwrap();
        let scope = EnclosingRubyScope::module(&scope);
        let spec = Spec::new(&mut interp, "Foo", c"Foo", Some(scope)).unwrap();
        let rclass = unsafe { interp.with_ffi_boundary(|mrb| spec.rclass().resolve(mrb)) }.unwrap();
        assert!(rclass.is_none());
    }
    ...
    fn rclass_for_root_module() {
        let mut interp = interpreter();
        let spec = Spec::new(&mut interp, "Kernel", c"Kernel", None).unwrap();
        let rclass = unsafe { interp.with_ffi_boundary(|mrb| spec.rclass().resolve(mrb)) }.unwrap();
    ...
    fn rclass_for_nested_module() {
        ...
        let scope = Spec::new(&mut interp, "Foo", c"Foo", None).unwrap();
        let scope = EnclosingRubyScope::module(&scope);
        let spec = Spec::new(&mut interp, "Bar", c"Bar", Some(scope)).unwrap();
        let rclass = unsafe { interp.with_ffi_boundary(|mrb| spec.rclass().resolve(mrb)) }.unwrap();
        assert!(rclass.is_some());
    }
    ...
    fn rclass_for_nested_module_under_class() {
        ...
        let scope = class::Spec::new("Foo", c"Foo", None, None).unwrap();
